        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form method="POST" id="userForm" action="{{ route('admin.users.update', $user->id) }}">
                @csrf
                {{ method_field('PATCH') }}
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="first_name" id="first_name" placeholder="{{ __('First Name') }}:" value="{{ old('first_name',$user->first_name) }}">
                  </div>
                  <div class="form-group">
                    <label for="last_name">{{ __('Last Name') }}:<span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="last_name" id="last_name" placeholder="{{ __('Last Name') }}" value="{{ old('last_name',$user->last_name) }}">
                  </div>
                  <div class="form-group">
                    <label for="username">{{ __('Username') }}:<span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="username" id="username" placeholder="{{ __('Username') }}" value="{{ old('username',$user->username) }}">
                  </div>
                  <div class="form-group">
                    <label for="email">{{ __('Email Address') }}:<span class="text-danger">*</span></label>
                    <input type="email" class="form-control" name="email" id="email" placeholder="{{ __('Email Address') }}" value="{{ old('email',$user->email) }}">
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                      ...
                      <input type="phone" class="form-control" name="phone" id="phone" placeholder="{{ __('Phone') }}" autocomplete="false" value="{{ old('phone',$user->phone) }}">
                  </div>
                  <div class=" form-group">
                  ...
                  <div class=" form-group">
                      ...
                      <select id="status" name="status" class="custom-select rounded-0" id="exampleSelectRounded0">
                          <option value="1"  {{(old('status',$user->status) === 1) ? 'selected="selected"':''}} >{{ __('Active') }}</option>
                          <option value="0" {{(old('status',$user->status) === 0) ? 'selected="selected"':''}} >{{ __('Inactive') }}</option>
                      </select>
                  </div>
                </div>
                <!-- /.card-body -->
